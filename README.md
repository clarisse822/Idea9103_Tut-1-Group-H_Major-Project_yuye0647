...
![An image of Night of the Museum](readmeImages/816af-SX6nL._AC_UF1000,1000_QL80_.jpg)
![An image of Toy Story](readmeImages/toy-story-3-2010-poster.jpg)
...
My personal code is entirely built upon the group code, with additional effects tailored to achieve the core theme of bringing the artwork to life during “nights without spectators.” Technically, I first use a day-night state variable **dayNightT** (_0 for daytime, 1 for nighttime_), then smoothly interpolate it to **targetDayNightT** using **lerp()**. This way, when the scene transitions between day and night, **applyDayNightFilter()** gradually overlays a semi-transparent dark blue filter instead of instantly switching between daytime and nighttime. During nighttime, I also draw multiple concentric **ellipses** at the mouse position to simulate moonlight illumination, though the final effect feels more like a flashlight beam.

I designed several night animations: First, **FlowBlock** objects are created for each road grid. Within **drawFlowingBlocks()**, their **offset** are continuously updated to generate small color blocks resembling “traffic flow” moving along horizontal and vertical roads. Next, treating large blocks as building windows, I adjusted their transparency using **frameCount** in **drawBlockLights()**, making each block flicker at different rhythms. Finally, during night mode with **glitchActive** enabled, I repeatedly sampled random fragments from **artCanvas** within the frame, redrawing them with offsets to create a glitchy, deconstructed, and misaligned aesthetic.

Regarding tools, I did not introduce any third-party libraries beyond p5.js, using only p5.js within the course scope alongside native _HTML/CSS/JavaScript_. For all referenced content, I clearly documented sources and functions as comments alongside the code, explaining how they work and why they were used here.