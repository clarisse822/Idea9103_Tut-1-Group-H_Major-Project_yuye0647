...
let flowParticles = [];     // Flowing particles generated by yellow grids (reserved for future use)
...
function draw() {
  ...
  if (dayNightT > 0.4) {
    // Mouse position mapped directly to the frame region
    moonLightX = mouseX;
    moonLightY = mouseY;
  }
  ...
  for (let y = 0, row = 0; y < sourceImage.height; y += SAMPLE_STEP, row++) {
    for (let x = 0, col = 0; x < sourceImage.width; x += SAMPLE_STEP, col++) {
      ...
      if (r > 240 && g > 240 && b > 240) {
        ...
        // Mark the yellow squares as “roads” and record them
...
function drawMoonLight() {
  ...
  push();
  // Layers of light centered at the current moonLightX/Y
